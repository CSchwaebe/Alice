...
export default function BiddingGameRules() {
  ...
  const gameRulesText = [
    "BIDDING",
    "",
    "You and your team will participate in a deadly auction.",
    "The one with the lowest bid in each round will be eliminated.",
    "",
    "Rules:",
  ...
  const styledText = displayText.split('\n').map((line, index) => {
    if (index === 0) {
      return `<span class="text-xl lg:text-2xl font-bold text-foreground tracking-widest">${line}</span>`;
